...
class Prodigy:
    ...
    def print_contacts(self, outfile=""):
        ...
        for res1, res2 in self.ic_network:
            ...
            # )
            _fmt_str = ("{0.resname:>5s} {0.id[1]}{0.id[2]:1} {0.parent.id:>3s} {1.resname:>5s} {1.id[1]}{1.id[2]:1} {1.parent.id:>3s}\n") #20241031 edit
    ...
    def print_pymol_script(self, outfile=""):
        ...
        for pair in self.ic_network:
            for r in pair:
                ...
                chains[r.parent.id].add(str(r.id[1]) + r.id[2]) #20241031 edit
